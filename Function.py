...
# Main Streamlit App
st.title("Steel Design Application")

# File upload
file_path = st.file_uploader("Upload H-Shape Database", type=['csv'])
file_path_mat = st.file_uploader("Upload Material Database", type=['csv'])

if file_path is not None and file_path_mat is not None:
    # Read CSV files
    df = pd.read_csv(file_path, index_col=0)
    df_mat = pd.read_csv(file_path_mat, index_col=0)
    
    # Generate selection lists
    section_list = df.index.tolist()
    section_list_mat = df_mat.index.tolist()
